...
J = np.linspace(-1, -0.2, 1)               #spin coupling constant
B = 0                                      #external magnetic field
M = 30                                      #lattice size MxN
N = 30
steps = 1000                                #number of evolution steps per given temperature

T = np.linspace(0.1, 0.7, 50)                 #temperature range as of README
...
@jit(nopython=True)
def step(A_dense, beta, num, J, rand_spins):
    ...
    for j in range(len(beta)):                      #raster trough temperatures
        ...
        while l <= steps:                               #evolve trough steps number of timesteps
            ...
            w.append(np.exp(-abs(E)*beta[j]*k_b))
...
def main():
    ...
    #get number of nodes
    n = num(G)
    #create random spin list
    spins = np.random.choice(np.asarray([-1, 1]), n)   #create random spins for nodes
    ...
    for a in range(len(J)):
        ...
        E_beta, M_beta, cv_beta, xi_beta, n = step(A_dense, beta, n, J[a], spins)

        #for normalization purposes
        n_normalize = n*np.ones(len(E_beta)) 

        #plot Energy and magnetisation per site as a function of temperature
        ax1.scatter(T, E_beta/n_normalize, label='J={:10.2f}'.format(J[a]))
        ax2.scatter(T, M_beta/n_normalize)
        ax3.scatter(T, cv_beta/n_normalize)
        ax4.scatter(T, xi_beta/n_normalize)

        print('{}/{}'.format(a+1, len(J)))
