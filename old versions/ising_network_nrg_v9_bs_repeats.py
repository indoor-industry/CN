...
def step(A_dense, beta, num):
    ...
    for j in range(len(beta)):              #raster trough temperatures
        ...
        for t in range(repeat):             #repeat and average over runs
            ...
            bsM_repeat = np.empty((nbstrap, repeat))
            bsE_repeat = np.empty((nbstrap, repeat))
            bs_var_M_repeat = np.empty((nbstrap, repeat))
            bs_var_E_repeat = np.empty((nbstrap, repeat))            
            for p in range(nbstrap):
                g = bootstrap(rep_mean_E)
                f = bootstrap(rep_mean_M)
                d = bootstrap(rep_var_E)
                c = bootstrap(rep_var_M)
                bsE_repeat[p] = g
                bsM_repeat[p] = f
                bs_var_E_repeat[p] = d
                bs_var_M_repeat[p] = c
            bsE_repeat_avg = bs_mean(bsE_repeat)
            bsM_repeat_avg = bs_mean(bsM_repeat)
            bs_var_E_repeat_avg = bs_mean(bs_var_E_repeat)
            bs_var_M_repeat_avg = bs_mean(bs_var_M_repeat)            

        avg_mean_E = mean(bsE_repeat_avg)                   #average over repeats
        avg_mean_M = mean(bsM_repeat_avg)
        avg_var_E = mean(bs_var_E_repeat_avg)
        avg_var_M = mean(bs_var_M_repeat_avg)
